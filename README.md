...
이 저장소는 이력서/경력 소개 페이지를 포함한 개인 포트폴리오 웹으로 사용할 수 있습니다.

- 로컬 실행: 정적 서버로 열기
  - macOS/Linux: `python3 -m http.server 3000`
  - 브라우저: http://localhost:3000
- 포트폴리오 페이지: `templates/pages/resume.html`
- 배포 방법(택1)
  - GitHub Pages: `docs/` 또는 루트 정적 파일 배포
  - Netlify/Vercel: 저장소 연결 후 빌드 없이 정적 배포
- PDF 출력: 브라우저에서 인쇄(Cmd+P)로 PDF 저장

프로젝트 구조나 빌드 도구를 변경하지 않고도 바로 포트폴리오로 사용할 수 있습니다.
